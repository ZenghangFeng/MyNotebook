...
"""
确定好输入数据，用户输入一段文本描述分析需求，Agent进行分析，并返回数据
...
# 模型调用的工具函数
############################################################
async def plot_scatter(file_name:str):
    """
    读取excel文件，根据表格中的数据绘图
    :param file_name:
    :return:
    """
    # file_name = "A20250317.xlsx"
    try:
...
# 定义智能体
############################################################
system_message="你是一个有用的助手，可以对用户提供的数据文件进行分析，可以进行的分析包括：1）绘制图像；2）分析绘制的图片并输出结论"
...
async def main() -> None:
    # await Console(agent.run_stream(task="根据 {A20250317.xlsx} 这个Excel里面的数据绘图，返回是否绘制成功的结果；如果绘图成功，请分析绘制的 {A20250317.png} 图片，判断数据的相关性"))
    await Console(agent.run_stream(
        task="请分析 {A20250317.xlsx} 这个Excel里面的两列数据的相关性"))
